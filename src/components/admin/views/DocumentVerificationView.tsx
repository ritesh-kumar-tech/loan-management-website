        ...
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4 animate-in zoom-in-95 duration-150">
            ...
            <div className="p-4 bg-slate-100 rounded-2xl border border-slate-200 space-y-3">
              ...
              {selectedDocItem.doc.fileUrl ? (
                <>
                  <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
                    {selectedDocItem.doc.fileUrl.match(/\.(png|jpe?g|webp|gif)$/i) || selectedDocItem.doc.fileUrl.startsWith('data:image') ? (
                      <img src={selectedDocItem.doc.fileUrl} alt={selectedDocItem.doc.title} className="h-56 w-full object-contain" />
                    ) : (
                      <iframe title={selectedDocItem.doc.title} src={selectedDocItem.doc.fileUrl} className="h-56 w-full" />
                    )}
                  </div>
                  <a
                    href={selectedDocItem.doc.fileUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="w-full inline-flex items-center justify-center gap-1.5 py-2 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold text-xs cursor-pointer"
                  >
                ...
                <div className="p-6 text-center space-y-2">
                  ...
                  <div className="p-3 bg-white rounded-xl border border-slate-200 text-xs font-mono text-rose-600">
